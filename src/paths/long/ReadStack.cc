...
public:
    BaseMetrics() { for (unsigned idx = 0u; idx != 4u; ++idx) mVals[idx].second = idx; }

    MetricType &val(unsigned idx) {
        AssertLt(idx, 4u);
        return mVals[idx].first;
    }

    int id(unsigned idx) {
        AssertLt(idx, 4u);
        return mVals[idx].second;
    }
...
void readstack::SortByPid(const int64_t pid1, const int i1, const int i2) {
    ...
    vec<triple<int64_t, int64_t, std::pair<Bool, int> > > data(Rows());
    for (int i = 0; i < Rows(); i++)
        data[i] = make_triple(offsetp[i], Pid(ident[i]),
                              std::make_pair(Rc2(ident[i]), Offset(ident[i])));
    SortSync(data, ident);
    if (ident[0] != i1) {
        for (int j = 0; j < Rows(); j++) {
            if (ident[j] == i1) {
                std::swap(ident[0], ident[j]);
                break;
            }
        }
    }
            ...
            for (int t = 0; t < top; t++)
                for (int j = top; j < Rows(); j++) {
                    if (Base(j, c) != Base(t, c) && Qual(j, c) >= min_q
                        && Qual(t, c) >= min_q && count[Base(t, c)] >= min_count) { suspect[j] = True; }
                }
        }
    }
}

void readstack::Print(std::ostream &out, const vec<vec<char>> &con, const int w) const {
    // Determine which reads have their partner placed.

    vec<std::pair<int64_t, int> > have;
    for (int i = 0; i < Rows(); i++)
...
void readstack::Print(std::ostream &out, const vec<basevector> &cons, const int w) const {
...
namespace {
    ...
    public:
        ...
        friend bool operator!=(MotifLoc const &ml1, MotifLoc const &ml2) { return !(ml1 == ml2); }
...
readstack::CorrectAllEM3(basevector &b, QualVec &q, int &trim_to, std::vector<double> &pfriend, bool debug) const {
    ...
    // trim_to currently set to read len
    run_EMEC3(this->bases_, this->quals_, b_tmp, q_tmp, pfriend, trim_to, debug, this->id_[0]);
...
void readstack::CorrectAll(basevector &b, QualVec &q, int &trim_to,
    ...
    for (int i = 0; i < Cols(); i++) {
        ...
        vec<std::pair<int, int> > competitors;
        for (int j = 1; j < 4; j++)
            mx.val(j) -= top[mx.id(j)];

        Bool OK = False;
        if (mx.val(0) >= min_win && mx.val(0) >= min_win_ratio * mx.val(1)
            && mx.val(1) <= max_lose) { OK = True; }
        if (verbose) {
#pragma omp critical
            {
                std::cout << "column " << i << ", read base = " << as_base(Base(0, i))
    ...
    for (int i = 0; i < Rows(); i++) {
        ...
        for (int64_t l = start; l < stop; l++) {
            ...
            if (BinMember(offset_id_rc2_orig,
                          make_triple(offset3, id3, rc3))) { continue; }
...
vec<int> GetOffsets1(const readstack &stack1, const readstack &stack2,
    ...
    basevector con1 = stack1.Consensus1(), con2 = stack2.Consensus1();
    ...
    MakeKmerLookup3(con12,
                    kmers_plus);                              // sorted kmer list <kmer, con12 index, basevec offset>
    vec<int> doffsets;
    for (int i = 0; i < kmers_plus.isize(); i++) {
        ...
        for (j = i + 1; j < kmers_plus.isize(); j++) {
            if (kmers_plus[j].first != kmers_plus[i].first)
                break;
        }                                         // [i,j) bracket a range of the same kmer
        for (int l1 = i; l1 < j; l1++) {
            ...
            for (int l2 = i; l2 < j; l2++) {
                ...
                doffsets.push_back(o);
            }
        }                            // great, push out the offset
        ...
        for (int p1 = 0; p1 < stack1.Cols(); p1++) {
            ...
            if (p2 >= 0 && p2 < stack2.Cols()) {
                if (stack1.Qual(0, p1) >= 30 && stack2.Qual(0, p2) >= 30
                    ...
                    if (verbosity >= 3) {
                        std::cout << "excluding offset " << o << " because it "
                                  << "creates Q30 mismatch "
                                  << "with founders" << std::endl;
                    }
                    bad = True;
                    break;
                }
     ...
     {    int o = offsets[i];
          ...
          {    int p2 = p1 - o;
               ...
               {    vec<int> count1(4,0), count2(4,0);
                    ...
                    if ( !consistent )
                    {    if ( verbosity >= 3 )
                         {    std::cout << "dump offset " << o << " for reads "
                                   << stack1.Id(0) << "/" << stack2.Id(0)
                                   << ", creates inconsistency in "
                                   << "columns " << p1 << "/" << p2 << std::endl;    }
                         bad = True;
                         break;    }    }    }
          if ( !bad )
            ...
            for (int j = 0; j < offsets.isize(); j++) {
                ...
                if (val1[j][p1] && val2[j][p2]) {
                    /*
                         if ( verbosity >= 2 )
                         {
                         std::cout << "offset " << offsets[j].offset << " invalidates "
...
void AddPartnersCore(const int top, readstack &stack,
    ...
    vec<std::pair<int64_t, Bool> > candidates;
    for (int i = 0; i < stack.Rows(); i++) {
